"""Pure numeric helpers for Chunk 1 spatial features (plan pose-feature-extraction §7)."""
...
def _pairwise_stats_m(world_xy: np.ndarray) -> tuple[float, float, float, float]:
    """Mean / std / min / max of pairwise Euclidean distances (metres), upper triangle only."""
...
def _net_distance_stats_m(world_xy: np.ndarray) -> tuple[float, float, float, float]:
    """Per-player distance to net line ``wy=0``: use ``|wy|`` metres; then mean/std/min/max."""
    n = int(world_xy.shape[0])
    if n == 0:
        return (float("nan"), float("nan"), float("nan"), float("nan"))
...
def _mocon_mean_std_max_m(world_xy: np.ndarray) -> tuple[float, float, float]:
    """MOCON-style distances from each foot to the side's centroid (metres)."""
    n = int(world_xy.shape[0])
    if n == 0:
...
def chunk1_spatial_dict(
    *,
    ...
    """Flat row fragment for Chunk 1 spatial columns only (caller merges with base row)."""
