...
- **Fetch the latest annotation for that clip.** Query `annotations` filtered by `clip_id`, ordered by `exported_at desc`, take the first row's `payload`. The payload shape is `{"label_studio_task": ..., "label_studio_annotation": ...}` (written by `data_labeling/push_annotations.py`).
