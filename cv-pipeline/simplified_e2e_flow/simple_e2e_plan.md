...
- **Filter low-confidence poses.** Before computing per-player cues, drop detections whose mean keypoint confidence is below `pose_conf_thresh` (default `0.30`). This reduces noisy squats/high-five counts.
...
- **Compute the per-frame features** (one row per yielded frame; column names and thresholds in the schema below):
    ...
    - `knee_angle_mean_deg` / `knee_angle_min_deg`: mean/min knee angle (degrees) across players, using hip-knee-ankle triplets when all three keypoints pass `kp_conf_thresh`.
    - `squat_count`: count of players with knee angle < 120° (rough squat cue).
    - `squat_ratio`: `squat_count / n_players_total`.
    - `wrists_above_shoulder_count`: count of players with a wrist above the shoulder line (wrist y < min shoulder y).
    - `high_five_pair_count`: number of wrist pairs above shoulders whose distance is < `1.2 * median(shoulder_width)` (celebration cue).
    - `flow_mag_mean`, `flow_mag_std`, `flow_mag_p90`, `flow_mag_p95`: optical flow magnitude statistics between consecutive sampled frames (global motion intensity).
    - `camera_side_centroid_speed_m`, `opposite_side_centroid_speed_m`: per-side centroid speed in metres/sec between consecutive sampled frames (player group motion).
    - `inter_centroid_speed_m`: change rate of the inter-side centroid distance (formation change).
...
- **Fetch the latest annotation for that clip.** Query `annotations` filtered by `clip_id`, ordered by `exported_at desc`, take the first row's `payload`. The payload shape is `{"label_studio_task": ..., "label_studio_annotation": ...}` (written by `data_labeling/push_timeline_annotation.py`).
...
| `knee_angle_mean_deg` | float | mean knee angle (deg) across players |
| `knee_angle_min_deg` | float | min knee angle (deg) across players |
| `squat_count` | int | players with knee angle < 120° |
| `squat_ratio` | float | squat_count / n_players_total |
| `wrists_above_shoulder_count` | int | wrists above shoulder line |
| `high_five_pair_count` | int | close wrist pairs above shoulders |
| `flow_mag_mean` | float | mean optical flow magnitude |
| `flow_mag_std` | float | std optical flow magnitude |
| `flow_mag_p90` | float | 90th pct optical flow magnitude |
| `flow_mag_p95` | float | 95th pct optical flow magnitude |
| `camera_side_centroid_speed_m` | float | camera-side centroid speed (m/s) |
| `opposite_side_centroid_speed_m` | float | opposite-side centroid speed (m/s) |
| `inter_centroid_speed_m` | float | inter-centroid distance change rate |
