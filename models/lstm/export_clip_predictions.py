...
from models.lstm.dataset import FeatureWindowDataset, clip_to_source_map, load_labels_by_clip  # noqa: E402
...
def export_clip_predictions(
    ...
    *,
    ...
    if not clip_id:
        raise ValueError("empty clip_id")
    ...
    ensure_features_for_clips(
        [clip_id],
        backbone=backbone,
        clip_to_source=clip_to_source_map(FRAME_LABELS_CSV),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--force-extract",
        action="store_true",
        help="Re-run CNN feature extraction for this clip",
    )
    return p.parse_args()
...
if __name__ == "__main__":
    args = parse_args()
    try:
        out_csv = export_clip_predictions(
            args.clip_id,
