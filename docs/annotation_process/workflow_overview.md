...
End-to-end flow: **YouTube → clips on S3 + DB → Label Studio → annotations in DB.** Narrative architecture: [annotation_schema_and_systems.md](annotation_schema_and_systems.md). Executable SQL only: [schema.md](../schema.md).
...
3. **`.env`:** copy the template file shared on **WhatsApp** into `.env` in the repo root and fill in values (shared AWS + Supabase credentials, your personal **`ANNOTATOR_NAME`**, etc.). `data_labeling/prep_videos.py` and `data_labeling/push_annotations.py` both read it via `python-dotenv`.
...
1. **Ingest:** from repo root, run `python data_labeling/prep_videos.py '<youtube_url>' --display-name 'Human title'`. That downloads, segments to 60 s / 30 fps clips, uploads to S3, upserts `source_videos` and `clips`, then removes `./workdir/{id}/`.
...
flowchart TD
  subgraph ingest [Ingest once per source]
    YT[YouTube URL]
    PREP[data_labeling/prep_videos.py]
    YT --> PREP
...
Manual **Google Sheet** updates (after prep, after push) keep a lightweight human ledger alongside automation.
