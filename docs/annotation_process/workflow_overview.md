...
1. **Ingest:** from repo root, run `python scripts/prep_videos.py '<youtube_url>' --display-name 'Human title'`. That downloads, segments to 60 s / 30 fps clips, uploads to S3, upserts `source_videos` and `clips`, then removes `./workdir/{id}/`.
...
6. **Push:** `python scripts/push_annotations.py /path/to/export.json` (optional `--dry-run` first). Requires `.env` Supabase + `ANNOTATOR_NAME`. Inserts one row per task per annotator (skips duplicates already in DB). See W3 in [annotation_schema_and_systems.md](annotation_schema_and_systems.md).
